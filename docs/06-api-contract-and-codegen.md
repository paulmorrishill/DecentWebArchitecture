...
Domain outcomes are carried by the typed errors in [03-backend-domain-and-ports](03-backend-domain-and-ports.md) § 3.1,
mapped to statuses at the entrypoint. Transport failures use the ordinary transport
statuses.
...
- is **never hand-edited** and is excluded from linting and formatting.
...
- Both files are generated by the same command as everything else, and committed in the
  same commit as the source change.
- CI regenerates them and **fails on any diff**, on every pull request.
- Run that check on pull requests specifically, not only on the integration branch —
  the deploy may not wait for anything else.
- A diff in one of these files is never waved through as "just codegen". Read it and
  confirm it matches the change you intended.

### 6.4 An unchanged generated artifact needs a reason, not an assumption

This is the subtlest failure in the whole pipeline. A privacy table that did not change
is the correct outcome for a change that adds no personal data. It is also the exact
outcome of a regeneration that was skipped. The diff cannot tell them apart and only
one is safe.

How to tell, cheapest first:

- **Sibling artifacts move together.** One command rewrites the route table, the
  manifest, and the privacy table. If a sibling changed and this one did not, the
  generator ran and the no-change is genuine. If *nothing* changed, you have learned
  nothing.
- **Otherwise regenerate**, and confirm the tree is clean *after* the run. Clean after a
  fresh regeneration is genuinely no change. Clean because you never ran it is a gap the
  diff cannot show.
